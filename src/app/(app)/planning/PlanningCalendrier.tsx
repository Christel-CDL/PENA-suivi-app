...
export type JourCalendrier = {
  ...
  jour: string; // "Lun", "Mar"…
  numero: string; // "22"
  moisLabel: string | null; // "Septembre" sur le 1er jour du mois affiché (ou le tout premier jour de la plage), sinon null
  estAujourdhui: boolean;
  weekEnd: boolean;
  sites: string[];
  taches: { id: string; nom: string; statut: string; siteCode: string }[];
};
...
export function PlanningCalendrier({ jours }: { jours: JourCalendrier[] }) {
  ...
  // Regroupe les jours consécutifs d'un même mois pour le bandeau au-dessus du
  // calendrier (un mois peut ainsi être affiché une seule fois, en largeur,
  // plutôt que réécrit dans la case du 1er jour du mois).
  const moisGroupes = useMemo(() => {
    const groupes: { key: string; label: string; count: number }[] = [];
    for (const j of jours) {
      if (j.moisLabel) groupes.push({ key: j.date, label: j.moisLabel, count: 1 });
      else if (groupes.length > 0) groupes[groupes.length - 1].count++;
    }
    return groupes;
  }, [jours]);
  ...
  return (
    <div className="space-y-2">
      ...
      <div
        ref={scrollRef}
        className="snap-x snap-mandatory overflow-x-auto overscroll-x-contain rounded-lg border border-slate-200 bg-white"
      >
          ...
          {jours.map((j) => (
            ...
            >
              ...
              <div className="min-h-24 space-y-1 p-1.5">
                {j.sites.map((label) => (
                  <p key={label} className="rounded-full bg-slate-100 px-1.5 py-0.5 text-center text-[11px] leading-tight text-slate-700">
                    {label}
                  </p>
                ))}
                {j.taches.map((t) => (
